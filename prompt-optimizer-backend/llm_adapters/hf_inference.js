...
class HFInferenceAdapter {
  constructor() {
    ...
    this.waitForModel = (process.env.HF_WAIT_FOR_MODEL || 'true') === 'true';
  }

  /**
   * Call HF Inference API with retry logic and error handling
  ...
  async callAdapter(prompt, opts = {}) {
    ...
    const requestConfig = {
      ...
      data: {
        inputs: prompt,
        parameters: {
          max_length: opts.max_tokens || 1000,
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (response.data && Array.isArray(response.data) && response.data[0]) {
          ...
          throw new Error('Invalid response structure from HF API');
        }
      } catch (error) {
        lastError = error;
  ...
  _mockResponse(prompt, opts, startTime) {
    const latencyMs = Date.now() - startTime;
    const mockText = `[MOCK_HF] Optimized: ${prompt.substring(0, 50)}...`;
  ...
  _logRequest(prompt, response, latencyMs, attempt, isMock = false) {
    ...
    console.log(`[HF_INFERENCE] ${JSON.stringify(logEntry)}`);
  }
  ...
  _logError(prompt, error, latencyMs) {
    ...
    console.error(`[HF_INFERENCE_ERROR] ${JSON.stringify(logEntry)}`);
  }
}

module.exports = new HFInferenceAdapter();
